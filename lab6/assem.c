...
/* delete same dst & src and redundant jmps */
AS_instrList AS_deleteRepeat(AS_instrList il, Temp_map col){
	AS_instrList cur = il, prev = NULL;
	for(; cur; cur = cur->tail){
		AS_instr inst = cur->head;
		AS_instr next;
		if(cur->tail)
			next = cur->tail->head;
		if(inst->kind == I_MOVE && 		//delete same dst and src
			Temp_look(col,inst->u.MOVE.src->head) ==
			 Temp_look(col, inst->u.MOVE.dst->head));
    	else if(inst->kind == I_OPER && !strncmp(inst->u.OPER.assem, "jmp", 3)	//delete jmp
      				&& next && next->kind == I_LABEL 
			  		&& inst->u.OPER.jumps->labels->head == next->u.LABEL.label)
        		printf("delete jmp: %s\n", inst->u.OPER.assem);
		else
			prev = AS_splice(prev, AS_InstrList(cur->head, NULL));
	}
	return prev;
}
